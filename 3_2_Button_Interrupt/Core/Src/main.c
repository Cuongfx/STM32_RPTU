...
/* USER CODE BEGIN PV */
static volatile uint8_t pin_changed = 0;
...
static uint8_t is_holding = 0;
static uint8_t click_count = 0;
static uint32_t time_press_start = 0;
static uint32_t time_first_click = 0;
static uint8_t  event_single_click = 0;
static uint8_t  event_double_click = 0;
...
static void Button_Handle(void)
{
  ...
  {
    pin_changed = 0;
    uint8_t state = HAL_GPIO_ReadPin(Button_GPIO_Port, Button_Pin);
    if (state != button_filter)
    {
      button_filter = state;
  ...
  if (is_debouncing && (HAL_GetTick() - time_debouncing) >= DEBOUNCE_MS)
  {
